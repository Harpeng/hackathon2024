...
const tasksAfterSearchAndFilter = computed(() => {
    return tasksAfterSearch.value.map(status => {
        ...
        const filteredTasks = status.items.filter((task) => {
            if (filter.value.responseId !== 0 && task.performer.id !== filter.value.responseId) {
                return false;
            }
            ...
            const dateFrom = filter.value.dateFrom || Math.floor(new Date().getTime() / 1000);
            const dateTo = filter.value.dateTo || Math.floor(new Date().getTime() / 1000);
